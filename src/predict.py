"""
Inference & Prediction Pipeline Module for Indian Liver Patient Disease Prediction System.
Loads saved LightGBM prediction pipeline (models/prediction_pipeline.joblib)
and performs prediction and risk probability calculation for clinical patient data.
"""
...
def ensure_models_dir(models_dir: str = "models"):
    """Creates directory for model artifacts if it doesn't exist."""
...
def save_model_pipeline(
    ...
    """
    Serializes and saves the complete prediction pipeline and feature schema to disk.
    """
...
def load_model_pipeline(pipeline_path: str = "models/prediction_pipeline.joblib") -> dict:
    """
    Loads saved model pipeline payload from disk.
    """
...
def normalize_input_data(raw_data: Union[Dict[str, Any], pd.DataFrame]) -> pd.DataFrame:
    """
    Normalizes diverse dictionary key conventions (e.g. JSON API vs Form field names)
    to match the exact expected training feature names.
    """
...
def predict_liver_disease(
    ...
    """
    Predicts liver disease outcome and calculates positive-class risk probability for a new patient.

    Expected 10 Clinical Features:
    - Age
    ...
    # Encode Gender if passed as string
    ...
    # Impute missing ratio if absent
    ...
    # Risk categories based on positive-class probability:
    # Below 30% -> Low, 30%-70% -> Moderate, Above 70% -> High
    if prob_pct > 70.0:
        risk_category = "High"
    elif prob_pct >= 30.0:
